...
def run_nightly_optimization(tickers=None, timeframe=None, param_grid=None, n_jobs=None):
    ...
    # Update base_case=true for each ticker with portfolio-optimized params
    for sym in ticker_symbols:
        p = current[sym]
        cursor.execute('''
            UPDATE strategy_parameters
            SET macd_fast = %s, bb_std = %s, bb_period = %s,
                win_rate = %s, sharpe_ratio = %s, total_return = %s, total_trades = %s,
                max_drawdown = %s, updated_at = NOW()
            WHERE ticker_id = (SELECT id FROM tickers WHERE symbol = %s)
            AND base_case = true
        ''', (
            p['macd_fast'], p['bb_std'], p['macd_fast'],
            float(pmetrics['win_rate']), float(pmetrics['sharpe_ratio']),
            float(pmetrics['total_return']), int(pmetrics['total_trades']),
            float(pmetrics['max_drawdown']), sym
        ))
    conn.commit()
