...
- **Ratchet now compares the last SETTLED HOURLY BAR close, not the live tick (2026-09-10, stock leg only)**: the backtest (`--exit ratchet-atr`) evaluates exits on bar closes, but the live executor compared against the instantaneous Alpaca quote — so transient intra-hour wicks (CNXN 09-08, GLW 09-10 sold @ $165.34 on a mid-hour dip against a ~$166.6 stop) fired the ratchet even though no bar ever closed below the stop. New `latest_hourly_closes()` (`executor.py`) reads the most recent captured hourly bar close from `tbl_scanner_tickers_1hour` (written ~10 min into the hour by `capture_hourly.py`) for the comparison; the peak/ATR math in `_compute_ratchet_stops` is unchanged. Live gap-up opens no longer false-trigger (AEHR 08-14 case mooted since hourly closes reflect the day's actual prices). Verified by 3 mocked end-to-end tests (bar-close-above + live-quote-below → hold; bar-close-below → sell + cooldown; ETF unchanged).
