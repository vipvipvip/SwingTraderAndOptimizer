...
def show_mtf(label, state_file, capital):
    ...
    print(f"{'─' * 72}")
    print(f"  {'Ticker':8s} {'Entry':>9s} {'Now':>9s} {'P&L $':>10s} {'P&L %':>8s}  Score")
    print(f"  {'─' * 58}")
    for sym in picks:
        info = scores.get(sym, {})
        entry = float(info.get('close', 0))
        score = info.get('score', 0)
        now = db_close(sym)
        if now and entry:
            pl = now - entry
            print(f"  {sym:8s} ${entry:<6.2f}  ${now:<6.2f}  ${pl:>+8.2f}  {(pl/entry)*100:>+7.2f}%  {score:>4.1f}")
        else:
            print(f"  {sym:8s} ${entry:<6.2f}  {'N/A':>9s}  {'N/A':>10s}  {score:>4.1f}")
...
def main():
    show_mtf('MTF Stock (default)', '../mtf/.mtf_state_stock.json', 100000)
    show_mtf('MTF Stock (min-score 5)', '../mtf/.mtf_state_min5_stock.json', 100000)
    show_mtf('MTF ETF', '../mtf/.mtf_state_etf.json', 100000)
    show_daily()
    show_summary()
