...
# to the HOURLY timeframe. IMPORTANT: set chart aggregation to e.g. 60 min.
...
# hourly tables are considered unreliable, see AGENTS.md).
# Mirrors the scanner's hourly bars exactly:
...
#   - peak = highest SETTLED hourly close since entry
#   - stop = Max(stop, peak - mult*ATR), monotone (reset=False): re-entry only
#     above the OLD stop (stop never resets on re-entry)
#   - the in-progress hour never moves peak/stop: the last bar's state is
#     overridden with the last COMPLETED hour's state (settled-bar rule — never
#     evaluate a live/partial bar)
declare upper;
...
# Monotone hourly-ratchet state machine (variant B, reset=False), evaluated on
# each hour's own settled data.
def rawLong;
def rawEntries;
def rawPeak;
def rawStop;
if atr <= 0 {
    ...
    # first gauge: enter, anchor peak/stop at this hour's close
    rawLong    = 1;
    rawEntries = 1;
    rawPeak    = close;
    rawStop    = close - mult * atr;
} else if rawLong[1] {
    # while long: ratchet peak/stop up, exit on settled hourly close <= stop
...
# Display state: freeze the current (in-progress) hour onto the last completed
# hour's state so partial data never advances peak/stop/entries.
...
# Axis labels (value frozen to the last SETTLED hour)
...
# live (in-progress hour) price warning — the mirror only exits on a SETTLED close <= stop
AddLabel(on and close < stop, "BELOW STOP (intrabar)", Color.RED);